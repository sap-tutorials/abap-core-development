...
>The Self-Service Migration tool is currently not supported in trial environments.
...
In order to connect from the self service migration tool running in the public internet to an on-premise database, the [SAP Cloud Connector](https://help.sap.com/docs/CP_CONNECTIVITY/cca91383641e40ffbe03bdc78f00f681/e54cc8fbbb571014beb5caaf6aa31280.html) is needed to be installed and configured.  
...
The self-service migration tool will then be able to check the compatibility of a selected database.
...
Here is an example compatibility report for the database that will be migrated in this tutorial.
...
When migrating a database, it is important to review any non-default settings and consider if any of these changes should also be made to the SAP HANA Cloud database.
...
4. An alternative method is to create a remote source from the on-premise database to the SAP HANA Cloud database and use an INSERT INTO statement.  An example is given below.  For additional details on creating remote source connections, see [Access Remote Sources with SAP HANA Database Explorer](hana-dbx-remote-sources).
    ...
    There are other options to move data from one database to another including Smart Data Integration that can be considered.  TODO, Gordon can you recommend a few links?
