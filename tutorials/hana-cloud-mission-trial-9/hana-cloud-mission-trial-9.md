...
Now that you have the role created and granted privileges to this role, it's time to grant this role to a user. Next, create the public user `report` that shall have read-only access to the calculation view.

1.	Go back to your tab with the **SAP HANA Database Explorer** and open a SQL Console by clicking on the SQL icon (![SQL](icon-sql.png)) at the top left corner. Make sure that the connection is opened for an user that has system privileges `ROLE ADMIN` and `USER ADMIN`, such as, database user `DBADMIN`.
...
```
    ...
    > Using the clause `NO FORCE_FIRST_PASSWORD_CHANGE` is not considered a security best practice! This option will only be used for the purpose of this tutorial to not mix up the original and changed password.

3.	Now that the new user `report` is created, grant the user access to the role `PublicAccessSchema`.
...
5. Replace the `<schema name>` with the calculation view schema that has been copied in the beginning of this tutorial.
